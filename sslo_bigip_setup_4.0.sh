#!/bin/bash
# Version 4.0 | 12/20/19 | F5 Global Training Services
#
# This script automates common Setup Utility tasks.
...
# root and admin passwords will not be in compliance with 8 charater requirement set in BIG-IP v15.1 
# New host name: bigipX.f5trn.com
# UCS file: bigipX_prepared.ucs
...
#
# Note: The script will NOT work on the special instructor BIG-IP system
# (LTM17) with management IP address 192.168.17.31. Nor will it work on a
# BIG-IP system that already has network configuration data on it as
# determined by checking for VLAN internal in bigip_base.conf. If configuration
# data is found or the third octet in the management IP address is 17, the
# script will stop.
#
#
# This script is distributed as part of the ATC Trainer Materials available on request at instructor@f5.com email 
#
# Separate versions exist for each version of BIG-IP courseware. This script is
# for use with BIG-IP v15.1 courses.
# 
# TO RUN THIS SCRIPT:
...
#       cat classroom_bigip_setup_4.0.txt | od -c
#
# 3. If you see " \ r \ n " characters in the output, run these commands to remove them:
#       tr -d "\r" < classroom_bigip_setup_4.0.txt > classroom_bigip_setup_4.0a.txt
#       mv classroom_bigip_setup_4.0a.txt classroom_bigip_setup_4.0.txt   (reply "yes" when prompted)

# 4. Make script executable: chmod 755 /shared/tmp/classroom_bigip_setup_4.0.txt 
# 5. From the /shared/tmp/ directory, execute the script:
#       ./classroom_bigip_setup_4.0.txt
#    
#
...
tmsh modify sys global-settings hostname "bigip${n}.f5trn.com"
tmsh modify sys ntp servers replace-all-with { 172.16.20.1 } 
tmsh mv cm device "bigip1" "bigip${n}.f5trn.com"
# adding a leading zero for station numbers 1-9
station=${n}
if [ $n -lt 10 ]; then
  station=0${n}
fi
...
tmsh create net self "10.1.30.3${n}" address "10.1.30.3${n}/24" traffic-group traffic-group-local-only vlan external allow-service add { tcp:22 tcp:443 }
tmsh create net self "10.1.30.33" address "10.1.30.33/24" traffic-group traffic-group-1 vlan external allow-service add { tcp:22 tcp:443 }
